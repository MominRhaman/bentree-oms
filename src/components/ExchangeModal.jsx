...
const ExchangeModal = ({ order, onClose, onConfirm, inventory }) => {
    // 1. Initialize with Old Products (User edits this list)
    ...
    // New Product Price (e.g. 2500)
    const newProductValue = newProductTotal - actualDiscountAmount;

    // B. Calculate Old Product Value (Without Delivery)
    // Formula: Previous Grand Total - Previous Delivery Cost
    const oldDeliveryCharge = Number(order.deliveryCharge || 0);
    const oldGrandTotal = Number(order.grandTotal || 0);
    const oldProductValue = oldGrandTotal - oldDeliveryCharge; // (e.g. 1760 - 60 = 1700)

    // C. Calculate Difference (Product Level)
    // (e.g. 2500 - 1700 = 800) OR (1500 - 1700 = -200)
    const productDifference = newProductValue - oldProductValue;

    // D. Final Calculation (Difference + New Delivery)
    // (e.g. 800 + 60 = 860) OR (-200 + 60 = -140)
    const finalAdjustment = productDifference + Number(newDeliveryCost || 0);

    // E. New Grand Total (For System Record: New Prod Value + New Delivery)
    const systemNewGrandTotal = newProductValue + Number(newDeliveryCost || 0);
    ...
    const handleSubmit = async (e) => {
        ...
        // 2. Adjust Inventory (Return Old, Deduct New)
        ...
        // 3. Determine Due Amount
        // If finalAdjustment is positive, that's what the customer owes.
        // If negative, it's a refund, so due is 0 (or strictly handled as negative).
        const newDueAmount = finalAdjustment > 0 ? finalAdjustment : 0;

        // 4. Update Order (Freezing old data in exchangeDetails)
        onConfirm(order.id, {
            ...
            deliveryCharge: Number(newDeliveryCost || 0),
            ...
            exchangeDetails: {
                ...
                priceDeviation: finalAdjustment, // Net Payable/Refund
                exchangeDate: new Date().toISOString().split('T')[0]
            },
            
            // Note for quick reference
            note: `Exchange. (New Val ${newProductValue} - Old Val ${oldProductValue}) + Del ${newDeliveryCost} = ${finalAdjustment}`
        });
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="text-xs text-slate-600 mb-4 bg-blue-50 p-3 rounded border border-blue-100">
                    ...
                    <p className="font-mono text-blue-700">(New Product Value - Old Product Value) + New Delivery Cost = Payable/Refund</p>
                </div>
