...
import { X, Edit2, Save, Trash2, Printer, MapPin, Phone, User, Package, Plus, Clock } from 'lucide-react';
import { disableScroll } from '../utils';

const OrderDetailsPopup = ({ order, onClose, getStatusColor, onEdit }) => {
    ...
    const saveChanges = () => {
        if (onEdit) {
            // Add a note to history indicating an edit occurred
            const updatedOrder = {
                ...editedOrder,
                history: [
                    ...(order.history || []),
                    {
                        ...
                        note: 'Order details edited manually',
                        updatedBy: 'User'
                    }
                ]
            };
            onEdit(order.id, order.status, updatedOrder);
            setIsEditing(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                    ...
                                    {isEditing ? (
                                        <>
                                            <div className="flex-1">
                                                ...
                                                <input className="w-full p-2 border rounded text-sm bg-white" placeholder="Code" value={p.code} onChange={e => handleProductChange(i, 'code', e.target.value)} />
                                            </div>
                                            <div className="flex gap-2">
                                                <div className="w-20">
                                            ...
                                            <div className="flex items-end sm:w-auto w-full mt-2 sm:mt-0">
                                                {editedOrder.products.length > 1 && (
                                                    ...
                                                    >
                                                        <Trash2 size={18} />
                                                        <span className="ml-2 text-sm font-medium sm:hidden">Remove Item</span>
                                                    </button>
                                                )}
                                            </div>
                                        </>
                    ...
                    {/* --- ORDER HISTORY / TRANSACTION LOG (SMART FILTERING) --- */}
                    <div className="pt-4 border-t border-slate-100">
                        ...
                        <ol className="relative border-l border-slate-200 ml-2">
                            {(order.history || []).map((h, i) => {
                                ...
                                // 2. FILTERING LOGIC
                                // If status is 'Pending' AND title is still just 'Pending' (no action detected),
                                // AND it's NOT the very first entry... HIDE IT.
                                if (h.status === 'Pending' && title === 'Pending' && i !== 0) {
                                    return null; 
                                }
                                ...
                                return (
                                    <li key={i} className="mb-6 ml-4">
                                        ...
                                        {/* Explicitly display collected money for Delivered status */}
                            ...
                            {(order.history || []).length === 0 && (
                                <li className="ml-4 text-xs text-slate-400">No history available.</li>
                            )}
                        </ol>
                    </div>
                ...
                <div className="p-4 border-t bg-slate-50 flex justify-between items-center">
                    {isEditing ? (
                        <>
                            <button onClick={() => { setIsEditing(false); setEditedOrder(JSON.parse(JSON.stringify(order))); }} className="px-4 py-2 text-slate-600 font-medium hover:bg-slate-200 rounded">Cancel</button>
                            <button onClick={saveChanges} className="px-6 py-2 bg-emerald-600 text-white font-bold rounded shadow hover:bg-emerald-700 flex items-center gap-2">
                                <Save size={18} /> Save Changes
                            </button>
                        </>
                    ) : (
                        <button className="w-full py-2 bg-slate-800 text-white font-bold rounded flex justify-center items-center gap-2 hover:bg-slate-900" onClick={() => window.print()}>
