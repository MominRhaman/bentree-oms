...
const DispatchTab = ({ orders, onUpdate }) => {
    ...
    // --- NEW LOGIC: Calculate group totals for bottom-to-top SL ---
    const counts = useMemo(() => {
        const pending = filtered.filter(o => o.status === 'Confirmed' || o.status === 'Exchanged').length;
    ...
    // --- NEW LOGIC: Initialize counters for the render loop ---
    let pendingSLIdx = 0;
    let dispatchedSLIdx = 0;
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-x-auto">
                <table className="w-full text-sm text-left min-w-[800px]">
                    <thead className="bg-slate-50 border-b">
                        <tr>
                            <th className="p-3 w-12 text-center text-slate-500 font-semibold">SL</th>
                    ...
                    <tbody>
                        {filtered.map((order) => {
                            ...
                            let currentSL = 0;
                            if (isPending) {
                                currentSL = counts.pending - pendingSLIdx;
                                pendingSLIdx++;
                            } else if (order.status === 'Dispatched') {
                                currentSL = counts.dispatched - dispatchedSLIdx;
                                dispatchedSLIdx++;
                            }
                        ...
                        {filtered.length === 0 && (
                            <tr>
                                <td colSpan="7" className="p-8 text-center text-slate-400">No dispatch orders found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
